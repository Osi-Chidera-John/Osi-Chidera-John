# Hi, I'm Osi Chidera John👋

**Healthcare Data Analyst & Researcher** | Founder of [**Jemva**]([https://www.linkedin.com/company/jemva/](https://www.linkedin.com/company/jemva/about/?viewAsMember=true)) | B.MLS Student at Nnamdi Azikiwe University

I build data-driven solutions, interactive dashboards, and automated analytics pipelines that turn complex raw data into actionable intelligence. My background spans business intelligence, empirical data analysis, and healthcare research.

---

### 🔬 Research & Academic Involvement

* 🧬 **Student Researcher | African Education Initiative (NEF Club - NAU Nnewi Chapter):** Collaborate on academic research workflows and assist university faculty with quantitative data processing and analysis.
* 🧪 **Independent Researcher:** Currently conducting an independent research study at the intersection of medical laboratory science and computational data analysis (manuscript in preparation).

---

###  My Skills & Tools

* **Languages & Querying:** Python (Pandas, NumPy, Matplotlib), SQL (MySQL)
* **BI & Reporting:** Power BI, DAX, Power Query, Microsoft Excel
* **Data Engineering & Workflow:** Git, GitHub, Data Cleaning & Transformation (ETL)
* **Core Domains:** Business Intelligence, Computational Healthcare & Research Analytics

---

### 🚀 What I'm Doing

* 💼 **Founding [Jemva]([https://www.linkedin.com/company/jemva/](https://www.linkedin.com/company/jemva/about/?viewAsMember=true)):** Delivering end-to-end analytics, automation, and BI solutions for growing businesses.
* 📈 **Open-Source Analytics:** Developing end-to-end data analytics portfolios, interactive dashboards, and research datasets.
* 🌱 **Continuous Learning:** Expanding capabilities in data engineering pipelines and statistical automation.


---

### 📬 Connect With Me

[![LinkedIn]((https://www.linkedin.com/in/john-chidera-jr/))
[![Email](mailto:chiderajohn519@gmail.com)
